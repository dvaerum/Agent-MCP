...
Wave 9 PR 1 of 7 in ``prancy-napping-pie.md``. Re-dispatched from
current main ``eb3133b`` after PR #253 was closed (stale base).
Migrates the four legacy role-tier admit-checks inside
...
identical — the helper is purely an internal refactor of the
dispatch.
...
asserting the cap string per role string and a defensive
``ValueError`` on unknown role strings.
"""
...
# Each row: (label, role, principal, expect_admit)
