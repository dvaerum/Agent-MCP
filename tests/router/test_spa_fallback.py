...
``/agent-mcp/app/<name>/`` rather than 404-ing, otherwise a
...
async def test_unknown_dashboard_path_serves_root_index(
    ...
    """``/agent-mcp/app/<name>/tasks/`` has no matching file on
...
async def test_unknown_dashboard_path_no_trailing_slash_also_falls_back(
    ...
    resp = await client.get(
        "/agent-mcp/app/foo/tasks", allow_redirects=False,
    )
...
async def test_dashboard_serves_real_file_when_present(
    ...
    resp = await client.get("/agent-mcp/app/foo/static-asset.txt")
