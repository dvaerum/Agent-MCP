"""Worker-facing clarity for edit/delete_task_note rejections.

Bug. ``task_notes_tools._classify_db_error`` collapses BOTH the DB
layer's "not found" and its "owned by <author>" ownership failure into a
bare ``NotFound(resource="task note")`` that renders as
``"task note '<id>' not found"``. A worker that is looking at a note
plainly present in ``view_tasks`` — but authored by someone else — is
told the note *doesn't exist*, reads that as a bug, and files a false
report. The message is honest about neither outcome.

Fix (worker-facing wording only). Fuse the two outcomes into ONE opaque
message that ADDS the author-only policy hint::

    task note '<id>' not found, or you are not its author. Only a note's
    original author (or an admin) can edit or delete it.

Two properties are load-bearing and pinned below:

* PF-1 (note-existence oracle): a FOREIGN-authored note and a
  NONEXISTENT note must yield the SAME message. The wording must not
  confirm the note exists, and it must never interpolate the authoring
  agent's id (the DB layer's ``owned by {author!r}`` string). Only the
  static author-only policy hint is added.
* The rejection stays a typed :class:`NotFound` (REST → 404,
  ``resource="task note"``) so the existing typed-contract guards in
...
# Kept in lock-step with ``task_notes_tools._AUTHOR_ONLY_HINT``.
_EXPECTED_HINT = (
...
async def test_edit_foreign_note_message_adds_author_only_hint(tmp_path) -> None:
    """A worker editing a note authored by someone else sees the fused
    author-only hint — not a bare "not found" that reads as a bug."""
    ...
    async with mcp_session(tmp_path) as admin:
        ...
        note_id = task_notes_db.add_note("wm-edit-foreign", "alice", "v1")
        ...
        assert msg == f"task note '{note_id}' not found{_EXPECTED_HINT}", msg
        assert "or you are not its author" in msg
        assert "Only a note's original author (or an admin) can edit or delete it." in msg
...
async def test_edit_foreign_and_missing_yield_same_message(tmp_path) -> None:
    """PF-1 note-existence oracle: a FOREIGN-authored note and a
    NONEXISTENT note produce the SAME opaque message (modulo the id the
    caller already supplied). A worker can't use the wording to confirm a
    foreign note exists, and no author id leaks."""
    from agent_mcp.db.actions import task_notes_db
    from agent_mcp.tools.registry import dispatch_tool_call
    ...
    async with mcp_session(tmp_path) as admin:
        ...
        await admin.create_worker("alice")
        bob = await admin.create_worker("bob")
        foreign_id = task_notes_db.add_note("wm-edit-oracle", "alice", "v1")
        missing_id = 999999

        p = _agent_principal("bob", bearer=bob.token)
        foreign = await dispatch_tool_call(
            "edit_task_note", {"note_id": foreign_id, "text": "x"}, principal=p,
        )
        missing = await dispatch_tool_call(
            "edit_task_note", {"note_id": missing_id, "text": "x"}, principal=p,
        )
...
async def test_delete_foreign_note_message_adds_author_only_hint(tmp_path) -> None:
    """delete_task_note carries the same fused author-only wording."""
    from agent_mcp.db.actions import task_notes_db
    from agent_mcp.tools.registry import dispatch_tool_call
    ...
    async with mcp_session(tmp_path) as admin:
        ...
        await admin.create_worker("alice")
        bob = await admin.create_worker("bob")
        note_id = task_notes_db.add_note("wm-del-foreign", "alice", "keep")

        p = _agent_principal("bob", bearer=bob.token)
        result = await dispatch_tool_call(
            "delete_task_note", {"note_id": note_id}, principal=p,
        )
        ...
        # Note not deleted.
        assert task_notes_db.get_note(note_id) is not None
