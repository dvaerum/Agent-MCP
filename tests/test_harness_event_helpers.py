"""Tests for the wait_for_event helper added to `tests/harness.py`
(plan Phase 5).
...
`read_inbox()`/`read_status()` (the `resources/read`-backed
counterparts) were tested here too until Phase F's `agent_mcp/resources/`
