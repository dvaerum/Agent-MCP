...
a future change to the base rule — e.g. the cross-agent task-visibility
toggle this module was built to support — lands in exactly one place
instead of requiring a class-sweep across 5 files.
...
def can_access_task(
    ...
    *,
    ...
    A missing ``requester_id`` degrades CLOSED: with no admin bypass it
    can only ever match ``include_unassigned`` (never an
    ``assigned_to``/``created_by`` equality, since ``None`` is
...
def sql_fragment(
    requester_id: Optional[str], can_view_all_tasks: bool
) -> Tuple[str, List[str]]:
    """Return the ``(sql_fragment, params)`` that scopes a ``tasks``
    ``SELECT`` to the exact-match half of :func:`can_access_task` (no
    ``include_created_by`` / ``include_unassigned`` widening).

    An admin/manager (``can_view_all_tasks``) gets an empty fragment
    (unscoped). Otherwise: ``AND assigned_to = ?`` bound to
    ``requester_id``. A missing/falsy ``requester_id`` degrades closed
    via an unsatisfiable ``AND 1=0`` — NOT ``assigned_to = ''``, which
    would accidentally match a real row whose ``assigned_to`` is
    itself the empty string (``is_unassigned`` treats "" as
    unassigned, so such rows exist) rather than matching nothing.
    """
    if can_view_all_tasks:
        return "", []
    if not requester_id:
