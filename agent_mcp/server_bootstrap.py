...
def get_admin_token_from_db(project_dir: str) -> Optional[str]:
    """Read the persisted admin token out of the project DB.
    ...
    the DB is absent or the row is missing — both are normal on first
    ...
    try:
        ...
        logger.error("Error reading admin token from database: %s", exc)
        return None
